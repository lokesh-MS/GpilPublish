        ...
        public ActionResult TransactionSave(string Crop, string Variety, string Organization, string TransactionType, string BankName, string Amount ,string transferTOOrgnList, string transferToBankList)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    {
                        strQuery = "SELECT VARIETY FROM GPIL_ORGN_MASTER WHERE ORGN_CODE = '" + Organization + "'";
                        ...
                        strQuery = "SELECT BG_AMOUNT,UTILIZED_AMOUNT FROM GPIL_BG_INFORMATION WHERE ORGN_CODE = '" + Organization + "' AND CROP='" + Crop + "' AND VARIETY='" + strTransferVariety + "'";

...
namespace GPILWebApp.ViewModel
{
    ...
    public class ReportManagement : GPIObject
    {
        ...
        public DataTable GetOrnCode1()
        {
            DataTable dt = new DataTable();
            try
            {
                string strQry = "select ORGN_CODE + ' - ' + ORGN_NAME as OrgnName, ORGN_CODE as OrgnCode from[dbo].[GPIL_ORGN_MASTER]  where orgn_type='factory'";
                dt = base.ODataServer.GetDataTable(strQry);
            }
            catch (Exception ex)
            {
...
;
        ...
        public DataTable GetFactoryLP5GradeShipmentDetails(string shipmentNo)
        {
            DataTable dt = new DataTable();
            try
            {
                string sql = "SELECT D.GRADE,ISNULL(R.Rate,0)RATE FROM GPIL_SHIPMENT_DTLS(NOLOCK) D LEFT OUTER JOIN GPIL_SHIPMENT_HDR(NOLOCK) H";
                sql = sql + " ON H.SHIPMENT_NO=D.SHIPMENT_NO LEFT OUTER JOIN GPIL_RATE_MASTER(NOLOCK) R ON H.SENDER_ORGN_CODE=R.Loc_Code AND D.GRADE=R.Item_Code";
                sql = sql + " AND CONVERT(VARCHAR(6),R.Last_Updated_Date,112)=CONVERT(VARCHAR(6),GETDATE(),112)";
                sql = sql + " WHERE H.SHIPMENT_NO='" + shipmentNo + "' ";
                sql = sql + " GROUP BY D.GRADE,R.Rate";
        ...
        public DataSet FACTORYGetLP5Details(string strShipmentNo)
        {
            DataSet ds = new DataSet();
            try
            {
                string sql = "";
                sql = "SELECT DISTINCT SHIPMENT_NO,SENDER_NO,SENDER_DATE,RECEIVER_NO,RECEIVED_DATE,SENDER_ORGN_CODE,RECEIVER_ORGN_CODE,SENDER_TRUCK_NO,SENT_BY,";
                sql = sql + "FRIEGHT_CHARGES,ISNULL(TBL2.Rate,0) AS FRIEGHT_RATE,CROP,VARIETY,GRADE,GRADE_DESC,MARK,CASES,GROSS_WT,NET_WT,ORDER_NO,RC_NO,";
                sql = sql + "DRIVER_NAME,DRIVING_LICENCE_NO,TRANSPORT_CODE,TRANSPORT_NAME,USER_NAME,SENDER,RECEIVER,ATTRIBUTE1,ATTRIBUTE2,ATTRIBUTE3,";
                sql = sql + "ATTRIBUTE4,(NET_WT*ISNULL(TBL2.Rate,0)) AS TOTAL_RATE,TBL1.Fr_GST,TBL1.Fr_State,TBL1.To_GST,TBL1.To_State,TBL1.SNO FROM";
                sql = sql + " (SELECT H.SHIPMENT_NO,H.SENDER_NO,CONVERT(NVARCHAR(21),H.SENDER_DATE,105) AS SENDER_DATE,H.RECEIVER_NO,H.RECEIVED_DATE,";
                sql = sql + "H.SENDER_ORGN_CODE,H.RECEIVER_ORGN_CODE,H.SENDER_TRUCK_NO,H.SENT_BY,'0' AS FRIEGHT_CHARGES,C.CropYearName AS CROP,";
                sql = sql + "V.VarietyName AS VARIETY,D.GRADE,ITEM_DESC AS GRADE_DESC,RIGHT(D.GRADE,4) AS MARK,COUNT(GPIL_BALE_NUMBER) AS CASES,";
                sql = sql + "SUM(CONVERT(FLOAT,ISNULL(tB.GrossWeight,'0'))) AS GROSS_WT,SUM(MARKED_WT) AS NET_WT,'' AS ORDER_NO,H.RC_NO,H.DRIVER_NAME,";
                sql = sql + "H.DRIVING_LICENCE_NO,T.TransporterCode AS TRANSPORT_CODE,T.TransporterName AS TRANSPORT_NAME,U.USER_NAME,";
                sql = sql + "(H.SENDER_ORGN_CODE + ' - ' + SO.ORGN_NAME+ ' - ' +ISNULL(SO.ORGN_ADDRESS2,'')+ ' - ' +ISNULL(SO.ORGN_ADDRESS3,'')+ ' - ' +ISNULL(SO.ORGN_ADDRESS4,'')+ ' - ' +ISNULL(SO.ORGN_ADDRESS5,'')+'-'+SO.PIN_CODE) AS SENDER,";
                sql = sql + "(H.RECEIVER_ORGN_CODE + ' - ' + ISNULL(RO.ORGN_NAME,'')+ ' - ' +ISNULL(RO.ORGN_ADDRESS2,'')+ ' - ' +ISNULL(RO.ORGN_ADDRESS3,'')+ ' - ' +ISNULL(RO.ORGN_ADDRESS4,'')+ ' - ' +ISNULL(RO.ORGN_ADDRESS5,'')+'-'+RO.PIN_CODE) AS RECEIVER,";
                sql = sql + "'' AS ATTRIBUTE1,'' AS ATTRIBUTE2,'' AS ATTRIBUTE3,ISNULL(I.ATTRIBUTE4,'')ATTRIBUTE4,SO.ATTRIBUTE5 AS Fr_State,SO.ATTRIBUTE6 AS Fr_GST,RO.ATTRIBUTE5 AS To_State,RO.ATTRIBUTE6 AS To_GST,ISNULL(G.SNO,1)SNO FROM GPIL_SHIPMENT_DTLS(NOLOCK) D,";
                sql = sql + "GPIL_SHIPMENT_HDR(NOLOCK) H,GPIL_ITEM_MASTER(NOLOCK) I,GPIL_ORGN_MASTER(NOLOCK) SO,GPIL_ORGN_MASTER(NOLOCK) RO,";
                sql = sql + "GPIL_USER_MASTER(NOLOCK) U,mTransporters(NOLOCK) T,mCropYears(NOLOCK) C,mVarieties(NOLOCK) V,tBarcodeDetails(NOLOCK) tB,GPIL_GST_INVOICE_NO(NOLOCK) G";
                sql = sql + " WHERE H.SHIPMENT_NO=D.SHIPMENT_NO AND tB.CaseBarCode=D.GPIL_BALE_NUMBER AND H.SHIPMENT_NO='" + strShipmentNo + "'";
                sql = sql + " AND I.ITEM_CODE=D.GRADE AND SO.ORGN_CODE=H.SENDER_ORGN_CODE AND RO.ORGN_CODE=H.RECEIVER_ORGN_CODE AND U.USER_ID=H.SENT_BY AND";
                sql = sql + " H.TRANSPORT_NAME=T.TransporterCode AND C.CropYearCode = LEFT(D.GPIL_BALE_NUMBER,1) AND V.VarietyCode=RIGHT(GPIL_BALE_NUMBER,4) AND H.SHIPMENT_NO=G.SHIPMENT_NO";
                sql = sql + " GROUP BY H.SHIPMENT_NO,H.SENDER_NO,H.SENDER_DATE,H.RECEIVER_NO,H.RECEIVED_DATE,H.SENDER_ORGN_CODE,H.RECEIVER_ORGN_CODE,";
                sql = sql + "H.SENDER_TRUCK_NO,H.SENT_BY,C.CropYearName,V.VarietyName,D.GRADE,ITEM_DESC,H.RC_NO,H.DRIVER_NAME,H.DRIVING_LICENCE_NO,";
                sql = sql + "T.TransporterCode,T.TransporterName,U.USER_NAME,SO.ORGN_NAME,RO.ORGN_NAME,I.ATTRIBUTE4,SO.ORGN_ADDRESS2,SO.ORGN_ADDRESS3,";
                sql = sql + "SO.ORGN_ADDRESS4,RO.ORGN_ADDRESS2,RO.ORGN_ADDRESS3,RO.ORGN_ADDRESS4,SO.ATTRIBUTE5,SO.ATTRIBUTE6,RO.ATTRIBUTE5,RO.ATTRIBUTE6,SO.ORGN_ADDRESS5,SO.PIN_CODE,RO.ORGN_ADDRESS5,RO.PIN_CODE,G.SNO)AS TBL1 LEFT OUTER JOIN";
                sql = sql + " (SELECT Loc_Code,Item_Code,Rate,Last_Updated_Date FROM GPIL_RATE_MASTER(NOLOCK)) AS TBL2 ON TBL1.SENDER_ORGN_CODE=TBL2.Loc_Code AND";
                sql = sql + " TBL1.GRADE=TBL2.Item_Code AND CONVERT(VARCHAR(6),TBL2.Last_Updated_Date,112)=CONVERT(VARCHAR(6),GETDATE(),112)";
        ...
        public DataTable BindGradeTransferReport(string strFromDate, string strTodate, string strOrgnCode, string strCrop, string strVariety)
        {
            ...
            try
            {
                ...
                if (strOrgnCode == "0")
                {
                    ...
                    strQuery = strQuery + " ISNULL(H.ATTRIBUTE3,'')<>'N' AND H.ORGN_CODE=O.ORGN_CODE AND SUBSTRING(D.GPIL_BALE_NUMBER,1,2)=C.CROP AND SUBSTRING ";
                    strQuery = strQuery + " (D.GPIL_BALE_NUMBER,3,2)=V.VARIETY AND H.RECIPE_CODE='RE-CLASSIFICATION' AND C.CROP='" + strCrop + "' AND ";
                    strQuery = strQuery + " V.VARIETY='" + strVariety + "' AND  H.CLASSIFICATION_DATE BETWEEN CONVERT(DATETIME,'" + strFromDate + " 00:00:00') ";
                    strQuery = strQuery + " AND CONVERT(DATETIME,'" + strTodate + " 23:59:59')+1 AND D.CLASSIFICATION_GRADE=I1.ITEM_CODE GROUP BY \r\n  ISSUED_GRADE, \r\n    CLASSIFICATION_GRADE,\r\n    H.ORGN_CODE + ' - ' + O.ORGN_NAME,\r\n    V.VARIETY + ' - ' + V.VARIETY_NAME,\r\n    C.CROP_YEAR,\r\n    I.ATTRIBUTE4,\r\n    I1.ATTRIBUTE4; ";
                    //strQuery = strQuery + " CLASSIFICATION_GRADE,H.ORGN_CODE+' - '+O.ORGN_NAME,V.VARIETY+' - '+V.VARIETY_NAME,C.CROP_YEAR,I.ATTRIBUTE4,I1.ATTRIBUTE4 ";
                    //strQuery = strQuery + " CLASSIFICATION_GRADE, H.ORGN_CODE + ' - ' + O.ORGN_NAME,  V.VARIETY + ' - ' + V.VARIETY_NAME,   C.CROP_YEAR,  I.ATTRIBUTE4, I1.ATTRIBUTE4; ";
                }
                else

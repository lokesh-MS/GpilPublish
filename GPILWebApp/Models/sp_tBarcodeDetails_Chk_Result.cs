
//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------


namespace GPILWebApp.Models
{

using System;
    
public partial class sp_tBarcodeDetails_Chk_Result
{

    public string LocCode { get; set; }

    public string CropYearCode { get; set; }

    public string PMRunNo { get; set; }

    public string CaseNo { get; set; }

    public string AccCaseNo { get; set; }

    public string LabelType { get; set; }

    public string GradeCode { get; set; }

    public string CaseCode { get; set; }

    public string VarietyCode { get; set; }

    public string SupplierCode { get; set; }

    public string CountryCode { get; set; }

    public string Moisture { get; set; }

    public string GrossWeight { get; set; }

    public string NetWeight { get; set; }

    public string LastCropYearCode { get; set; }

    public string LastGradeCode { get; set; }

    public string LastVarietyCode { get; set; }

    public Nullable<byte> Status { get; set; }

    public Nullable<System.DateTime> PrintedOn { get; set; }

    public string PrintedBy { get; set; }

    public Nullable<System.DateTime> LastRePrintedOn { get; set; }

    public string LastRePrintedBy { get; set; }

    public Nullable<System.DateTime> ModifiedOn { get; set; }

    public string ModifiedBy { get; set; }

    public string CaseBarCode { get; set; }

    public string VoidBarCode { get; set; }

    public Nullable<bool> IsExistingStock { get; set; }

    public Nullable<byte> D_Stts { get; set; }

    public string DateOfPacking { get; set; }

    public System.Guid rowguid { get; set; }

}

}

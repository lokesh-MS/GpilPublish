        ...
        public JsonResult UpdateFarmerPurchase(FARM_PURCHS_DTLS gPIL_TAP_FARM_PURCHS_DTLS)
        {
            ...
            try
            {
                ...
                if (ds1.Rows.Count > 0)
                {
                    ...
                    dFreight = Convert.ToDouble(RATE);
                    RATE = dFreight;
                }
                ds1.Clear();
           ...
           // b = MstrMgt.UpdateUsingExecuteNonQueryList(lstQry1);
            ...
            if (lstQry.Count > 0)
            {
                ...
                { 
                    ...
                    // strsql = "UPDATE GPIL_STOCK SET STATUS='Y', PROCESS_STATUS='N',SUBINVENTORY_CODE='CL' WHERE BATCH_NO = (SELECT HEADER_ID FROM GPIL_TAP_FARM_PURCHS_HDR WHERE PURCH_DOC_NO = '" + poNumber + "');";
                    strsql = "UPDATE GPIL_STOCK SET STATUS='Y', PROCESS_STATUS='N',SUBINVENTORY_CODE='CL' WHERE BATCH_NO = (SELECT HEADER_ID FROM GPIL_TAP_FARM_PURCHS_HDR WHERE PURCH_DOC_NO = '" + poNumber + "') AND STATUS='I' AND PROCESS_STATUS='Y';";
                    lststr.Add(strsql);

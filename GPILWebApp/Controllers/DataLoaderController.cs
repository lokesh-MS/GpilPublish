            ...
            try
            {
                ...
                for (int d = 0; d < purdata.Tables.Count; d++)
                {
                    ...
                    for (int h = 0; h < purdata.Tables[d].Rows.Count; h++)
                    {
                        ...
                        strsql = strsql + "Values('" + temphdr + "','SH" + sendorg + DateTime.Now.ToString("yyyyMMddhhmmss") + d + h + "','" + baleno.Trim() + "','" + markedwt.Trim() + "','" + diswt.Trim() + "','" + fromsubcode.Trim() + "','" + tosubcode.Trim() + "','" + Convert.ToDateTime(loadtime).ToString("yyyy-MM-dd HH:mm:ss") + "','INT','Y','" + Session["userID"].ToString() + "',GETDATE(),'" + grade.Trim() + "','" + attribute2.Trim() + "')";
                        //dlMgt.UpdateUsingExecuteNonQuery(strsql);
                        lstQry.Add(strsql);

                        strsql = "update GPIL_STOCK set STATUS='INT',PROCESS_STATUS='Y',CURR_ORGN_CODE='" + recevorg + "',BATCH_NO='" + temphdr + "',CURR_WT='" + diswt + "',LAST_UPDATED_BY='" + Session["userID"].ToString() + "',LAST_UPDATED_DATE=GETDATE() WHERE GPIL_BALE_NUMBER='" + baleno.Trim() + "'";
                        //dlMgt.UpdateUsingExecuteNonQuery(strsql);
                        lstQry.Add(strsql);
